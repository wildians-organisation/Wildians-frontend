...
export default function DetailsDons({ details }) {
  return (
    <div>
      <strong className="text-gray-700 font-medium">Repartition donations</strong>
        ...
        <table className="text-gray-700 w-full">
          ...
          <tbody>
            {Array.from(details).map(([address, donations]) => (
              <tr key={address}>
                <td className="border-t border-gray-200 py-2 text-center text-xs">{address}</td>
                <td className="border-t border-gray-200 py-2 text-center text-xs">
                  {Object.entries(donations).map(([charity, amount]) => (
                    <div key={charity}>
                      {charity}: {amount}
                    </div>
                  ))}
